...
def sentenceExtractor():
	...
	#for file in tqdm(list(Path("C:/Users/karam/PycharmProjects/skillExtraction/metajob").glob("*.sents"))):
		#file.unlink()

	for file in tqdm(list(Path("C:/Users/karam/PycharmProjects/skillExtraction/metajob").glob("*.clntxt"))):
		if not file.with_suffix(".sents").exists():
			with file.open("rt", encoding="utf8") as f:
				...
				with file.with_suffix(".sents").open("wt", encoding="utf8") as rf:
					for s in chain.from_iterable(sents):
						rf.write(s)
						rf.write("\n")
